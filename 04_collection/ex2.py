...
ans = ["짝수" if x % 2 == 0 else "홀수" for x in range(1, 11)]
print(ans)
...
ans = [len(x) for x in names]
print(ans)

# 길이가 5 이상인 이름만 뽑기

ans = [x for x in names if len(x) >= 5]
print(ans)
...
scores = zip(scores[1], scores[0], scores[2])
# result = [round(sum(x)/len(x),2) for x in scores]
